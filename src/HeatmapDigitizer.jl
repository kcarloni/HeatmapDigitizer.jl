...
"""
    ...
    digitize_map( map_path, scale_path, map_bins, vlims )

`map_path`, `scale_path` are the paths to image files of the (cropped!) heatmap and colorbar.

`map_bins` should be a tuple of the number of bins in the (x, y) dimensions.
`vlims` are the extrema of the color bar values. 
"""
